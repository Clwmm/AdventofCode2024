...
void checkBoundariesAndInsert(Grid& grid, const Position& antinode, std::set<Position>& antinodesPositions) {
    ...
    for (auto antenna : antennas) {
        ...
        for (size_t i = 0; i < positions.size(); ++i) {
            for (size_t j = i+1; j < positions.size(); ++j) {
                ...
                Position antinode_1 = {a.x + difference.x, a.y + difference.y};
                Position antinode_2 = {b.x - difference.x, b.y - difference.y};
                checkBoundariesAndInsert(grid, antinode_1, antinodesPositions);
                checkBoundariesAndInsert(grid, antinode_2, antinodesPositions);
            }
        }
